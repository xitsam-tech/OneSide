export default async function handler(req, res) {
  ...
  try{
    ...
    const input = text.slice(0, 8000); // cap length
    const tgt = String(to||'en').slice(0,8);
    const useModel = model || process.env.OPENAI_MODEL || 'gpt-4o-mini';
    const prompt = `Übersetze präzise ins ${tgt}. 
- Erhalte Zahlen, Währungssymbole, Prozent und Eigennamen.
- Behalte Zeilenumbrüche, Markdown/HTML inline-Tags (ohne neue zu erfinden).
